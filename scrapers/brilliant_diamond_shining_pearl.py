#!/usr/bin/env python3
"""
Scraper for Pokémon Brilliant Diamond & Shining Pearl (WikiDex).
Downloads the Sinnoh and extended Sinnoh Pokédexes.
Output: backend/data/games/bdsp.json  (see scrapers/README.md)
"""
...
SOURCE_URL = "https://www.wikidex.net/wiki/Lista_de_Pok%C3%A9mon_seg%C3%BAn_la_Pok%C3%A9dex_de_Sinnoh"
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                           "backend", "data", "games", "bdsp.json")
...
def parse_table(table_html):
    ...
    for row in rows:
        ...
        if re.match(r'^\d+$', first_content) and n >= 4:
            nac = first_content
            sin = cell_text(cells[1][1])

            rowspan_match = re.search(r'rowspan="(\d+)"', cells[0][0])
            if rowspan_match and int(rowspan_match.group(1)) > 1:
                pending_nac, pending_sin = nac, sin
            else:
                pending_nac = pending_sin = pending_name = None
            ...
            if len(icons) > 1:
                for form_name, icon_url in icons:
                    entries.append({
                        "nac": pending_nac, "dex_num": pending_sin,
                        "name": form_name or name,
                        "tipo1": tipo1, "tipo2": tipo2 or "", "icon_url": icon_url,
                    })
            else:
                icon_url = icons[0][1] if icons else None
                entries.append({
                    "nac": pending_nac, "dex_num": pending_sin, "name": name,
                    "tipo1": tipo1, "tipo2": tipo2 or "", "icon_url": icon_url,
                })
