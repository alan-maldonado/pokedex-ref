...
Downloads the Paldea Pokédex including regional variants and alternate forms.
...
def parse_table(table_html):
    ...
    for row in rows:
        ...
        if re.match(r'^\d+$', first_content) and n >= 4:
            nac = first_content
            pal = cell_text(cells[1][1])

            rowspan_match = re.search(r'rowspan="(\d+)"', cells[0][0])
            if rowspan_match and int(rowspan_match.group(1)) > 1:
                pending_nac, pending_pal = nac, pal
            else:
                pending_nac = pending_pal = pending_name = None
            ...
            if len(icons) > 1:
                for form_name, icon_url in icons:
                    entries.append({
                        "nac": pending_nac, "dex_num": pending_pal,
                        "name": form_name or name,
                        "tipo1": tipo1, "tipo2": tipo2 or "", "icon_url": icon_url,
                    })
            else:
                icon_url = icons[0][1] if icons else None
                entries.append({
                    "nac": pending_nac, "dex_num": pending_pal, "name": name,
                    "tipo1": tipo1, "tipo2": tipo2 or "", "icon_url": icon_url,
                })
...
def main():
    ...
    paldea = parse_table(tables[0])
    print(f"  Paldea: {len(paldea)} entries (including variants)")
